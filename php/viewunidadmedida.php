...
<section class="container">
	...
	<section class="content">
		...
		if($radio==1 && $filtro!="" && (int)$filtro){
			$sql.=" WHERE pkunidad = '".$filtro."' ";
		}
		if($radio==2 && $filtro!="" && $filtro){
			$sql.=" WHERE UPPER(uni_descrip) LIKE UPPER('%".$filtro."%') ";
		}
		$sql.=" ORDER BY ".$orden." ".$type." LIMIT ".$pagtam." OFFSET ".$inicio;
		$res=connector2($sql);
		$arrayhead=array('Tareas','PK','Descripción');
		$arraydb=array('pkunidad','pkunidad','uni_descrip');
		$title="TABLA UNIDAD DE MEDIDA";
		$del="frmdelunidadmedida.php";
		$frm="frminsunidadmedida.php";
		$filtro1="PK";
		$filtro2="Descripción";
		$filtro3="";
		$filtro4="";
