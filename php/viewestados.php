...
<section class="container">
	...
	<section class="content">
		...
		if($radio==1 && $filtro!="" && (int)$filtro){
			$sql1.=" WHERE pkestado = '".$filtro."' ";
		}
		if($radio==2 && $filtro!="" && $filtro){
			$sql1.=" WHERE UPPER(es_descrip) LIKE UPPER('%".$filtro."%') ";
		}
		...
		if($radio==1 && $filtro!="" && (int)$filtro){
			$sql.=" WHERE pkestado = '".$filtro."' ";
		}
		if($radio==2 && $filtro!="" && $filtro){
			$sql.=" WHERE UPPER(es_descrip) LIKE UPPER('%".$filtro."%') ";
		}
		$sql.=" ORDER BY ".$orden." ".$type." LIMIT ".$pagtam." OFFSET ".$inicio;
		$res=connector2($sql);
		$arrayhead=array('Tareas','PK','Descripción');
		$arraydb=array('pkestado','pkestado','es_descrip');
		$title="TABLA ESTADOS";
