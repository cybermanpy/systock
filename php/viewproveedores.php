...
<section class="container">
	...
	<section class="content">
		...
		$table="proveedores";
		$array=array('pkprov','prov_nombre','prov_telefono','prov_direccion','prov_correo');
		$sql1=select($table,$array);
		if($radio==1 && $filtro!="" && (int)$filtro){
			$sql1.=" WHERE pkprov = '".$filtro."' ";
		}
		if($radio==2 && $filtro!="" && $filtro){
			$sql1.=" WHERE UPPER(prov_nombre) LIKE UPPER('%".$filtro."%') ";
		}
		...
		if($radio==1 && $filtro!="" && (int)$filtro){
			$sql.=" WHERE pkprov = '".$filtro."' ";
		}
		if($radio==2 && $filtro!="" && $filtro){
			$sql.=" WHERE UPPER(prov_nombre) LIKE UPPER('%".$filtro."%') ";
		}
		$sql.=" ORDER BY ".$orden." ".$type." LIMIT ".$pagtam." OFFSET ".$inicio;
		$res=connector2($sql);
		$arrayhead=array('Tareas','PK','Proveedores','Telefonos','Direcciones','Correos');
		$arraydb=array('pkprov','pkprov','prov_nombre','prov_telefono','prov_direccion','prov_correo');
