<?php
function page($res,$total,$pagetam,$page,$inicio,$arraydb,$arrayhead,$title,$index,$frm,$orden,$type,$tableparte,$arrayparte,$arraypartedb,$arraypartehead,$whereparte,$user,$pass,$show){
	...
	while($row=fetchArray($res))
  {
		 ...
		 for($idb=0;$idb<count($arraydb);$idb++)
     {
			 switch($idb)
       {
				case 0:
					?>
          <td><a onClick="popup('<?=$frm?>?pk=<?=$row[$arraydb[$idb]]?>');return false;" href=""><img class="img" src="<?=$printer?>"></a></td>
          <?php
				break;
