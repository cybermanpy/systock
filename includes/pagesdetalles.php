            ...
            <table>
              <tr bgcolor="<?=$head ?>">
                ...
                for($iparte=0;$iparte<count($arraypartehead);$iparte++){
									if($iparte==0){
										?>
                    <th><a href="<?=$insdet?>?pk=<?=$row[$arraydb[$idb]]?>"><img class="img" src="<?=$plus?>"></a></th>
                    <?php
									}else{
										?>
              ...
              while($rowparte=fetchArray($resparte))
              {
                ...
                <tr bgcolor="<?=$color?>">
                  ...
                  for($ipartedb=0;$ipartedb<count($arraypartedb);$ipartedb++)
                  {
                    ...
                    {
											?>
                      <td><a href="<?=$updet?>?pk=<?=$rowparte[$arraypartedb[$ipartedb]]?>"><img class="img" src="<?=$pencil?>"></a> <a href="<?=$deldet?>?pk=<?=$rowparte[$arraypartedb[$ipartedb]]?>"><img class="img" src="<?=$cross?>"></a></td>
                      <?php
										}
                    else
                    {
